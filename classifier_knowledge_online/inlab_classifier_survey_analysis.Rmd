...
# if general, delete row
# replace with row with same participant_code, item_name, item_number == "second"
data_combination <- subset(data, item_number == "first")
for (row in data_combination) {
  this_participant_code <- row$participant_code
  this_item_name <- row$item_name
  if (row$classifier_type != "general") {
    
  }
}

# second approach
# start with specific classifiers
# if repeat, delete item_number == "second"
data_specific <- subset(data, classifier_type != "general")


# count most frequently used CL and relative frequency, all classifiers, sorted by item
CL_both_all_specific <- data %>% ###correct to name of new data frame
  group_by(.dots=c("item_name", "response")) %>% 
  tally(name = "count") %>% 
  mutate(freq = count / sum(count))
```
